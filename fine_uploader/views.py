...
from fine_uploader.signals import file_uploaded
...
class UploadView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            dest = handle_upload(request.FILES['qqfile'], form.cleaned_data)
            file_uploaded.send(sender=self.__class__, path=dest, request=request)
...
def handle_upload(f, fileattrs):
    ...
    # Chunked
    #if 'qqtotalparts' in fileattrs and (int(fileattrs['qqtotalparts']) > 1):
    #    chunked = True
    ...
    basewidth = 300
    thumb_dest = os.path.splitext( dest )[0] + '.thumbnail'
    ...
    thumb.save( thumb_dest + ".jpg", "JPEG")
